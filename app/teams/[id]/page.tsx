...
export default function TeamDetailPage() {
  ...
  const progressGap =
    Math.round((salesRate - expectedProgress) * 10) / 10;
  const salesRemaining = Math.max(0, salesTarget - totals.sales);

  const champagneRate = rate(totals.champagne, champagneGoal);
  const champagneRemaining = Math.max(0, champagneGoal - totals.champagne);

  const visitRate = rate(totals.visits, visitGoal);
  const visitRemaining = Math.max(0, visitGoal - totals.visits);
    ...
    <main className="min-h-screen bg-black text-white pb-24">
      <div className="mx-auto w-full max-w-md px-5 pt-8">
        ...
        <section className="rounded-3xl border border-zinc-800 bg-zinc-950 p-5">
          ...
          {salesTarget > 0 && expectedProgress > 0 && (
            <div className="mt-4 rounded-2xl border border-zinc-800 bg-black px-4 py-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-zinc-500">
                    進捗基準
      ...
      : `超過配分 ${Math.abs(unallocatedVisits)}組`}
        ...
        <section className="mt-6">
          ...
          <div className="mt-4 space-y-3">
            {[...members]
              .sort((a, b) => {
                const goalA = memberGoals.find(
                  (row) => row.member_id === a.id
                );
                const goalB = memberGoals.find(
                  (row) => row.member_id === b.id
                );

                const targetA = Number(goalA?.target_sales ?? 0);
                const targetB = Number(goalB?.target_sales ?? 0);

                const salesA = results
                  .filter((row) => row.member_id === a.id)
                  .reduce(
                    (sum, row) => sum + Number(row.sales ?? 0),
                    0
                  );

                const salesB = results
                  .filter((row) => row.member_id === b.id)
                  .reduce(
                    (sum, row) => sum + Number(row.sales ?? 0),
                    0
                  );
                ...
                const rateA =
                  targetA > 0 ? salesA / targetA : 999;
                const rateB =
                  targetB > 0 ? salesB / targetB : 999;
