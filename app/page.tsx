...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-black text-white pb-24">
      ...
      <nav className="fixed bottom-0 left-0 right-0 border-t border-zinc-900 bg-black/95"><div className="mx-auto grid max-w-md grid-cols-4"><Link href="/" className="py-4 text-center text-xs font-bold text-white">ホーム</Link><Link href="/members" className="py-4 text-center text-xs text-zinc-600">メンバー</Link><Link href="/daily" className="py-4 text-center text-xs text-zinc-600">日報</Link>{storeView ? <Link href="/settings" className="py-4 text-center text-xs text-zinc-600">設定</Link> : <span className="py-4 text-center text-xs text-zinc-800">設定</span>}</div></nav>
    </main>
  );
}
