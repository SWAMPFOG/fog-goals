...
export default function OrganizationPage() {
  ...
  async function disableTeam(team: {
    id: string;
    name: string;
  }) {
    ...
    if (error) {
      window.alert("無効化エラー: " + error.message);
      return;
    }
  ...
  async function editTeam(team: {
    ...
    const name = window.prompt("チーム名を変更", team.name);
    if (name === null || !name.trim()) return;

    const currentDepartment =
      departments.find((d) => d.id === team.department_id)?.name ?? "";

    const departmentNameInput = window.prompt(
      "所属部署名を入力",
      currentDepartment
    );
    if (departmentNameInput === null) return;

    const department = departments.find(
      (d) => d.name === departmentNameInput.trim()
    );

    if (!department) {
      window.alert("その部署は見つかりません");
      return;
    }
    ...
    if (error) {
      window.alert("更新エラー: " + error.message);
      return;
    }
  ...
  async function editMember(member: {
    ...
    const name = window.prompt("メンバー名を変更", member.name);
    if (name === null || !name.trim()) return;

    const currentTeam =
      teams.find((t) => t.id === member.team_id)?.name ?? "";

    const teamNameInput = window.prompt(
      "所属チーム名を入力",
      currentTeam
    );
    ...
    const team = teams.find(
      (t) => t.name === teamNameInput.trim()
    );

    if (!team) {
      window.alert("そのチームは見つかりません");
      return;
    }
...
<div className="flex items-start justify-between gap-4">
  ...
  <div className="flex shrink-0 gap-2">
    ...
    <button
      type="button"
      onClick={() => disableTeam(team)}
      className="rounded-xl border border-red-900 px-4 py-2 text-sm font-bold text-red-400"
    >
      無効化
    </button>
  </div>
