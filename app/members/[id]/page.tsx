...
export default function MemberDetailPage() {
  ...
  const mustSalesTarget = goal?.must_sales ?? 0;
  const mustSalesRate =
    mustSalesTarget > 0
      ? Math.min(100, Math.round((currentSales / mustSalesTarget) * 100))
      : 0;
  const mustSalesRemaining = Math.max(0, mustSalesTarget - currentSales);

  const salesRemaining = Math.max(0, salesTarget - currentSales);

  const champagneTargetValue = goal?.champagne_target ?? 0;
  const champagneRate =
    champagneTargetValue > 0
      ? Math.min(100, Math.round((currentChampagne / champagneTargetValue) * 100))
      : 0;
  const champagneRemaining = Math.max(
    0,
    champagneTargetValue - currentChampagne
  );

  const visitTargetValue = goal?.visit_count_target ?? 0;
  const visitRate =
    visitTargetValue > 0
      ? Math.min(100, Math.round((currentVisits / visitTargetValue) * 100))
      : 0;
  const visitRemaining = Math.max(0, visitTargetValue - currentVisits);
  ...
  return (
    <main className="min-h-screen bg-black text-white pb-24">
      <div className="mx-auto w-full max-w-md px-5 pt-8">
        ...
        {goal && goal.must_sales > 0 && (
          <>
            <div className="mt-4 h-2 overflow-hidden rounded-full bg-zinc-800">
              <div
                className="h-full rounded-full bg-white"
                style={{ width: `${mustSalesRate}%` }}
              />
            </div>
            <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-zinc-500">
              <span>現在 {yen(currentSales)}</span>
              <span className="text-center">達成率 {mustSalesRate}%</span>
              <span className="text-right">残り {yen(mustSalesRemaining)}</span>
            </div>
          </>
        )}
              ...
              <div className="mt-6 border-t border-zinc-900 pt-5">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-zinc-500">
                  <span>現在 {yen(currentSales)}</span>
                  <span className="text-center">達成率 {salesRate}%</span>
                  <span className="text-right">残り {yen(salesRemaining)}</span>
                </div>
              </div>
            </section>
            ...
            <section className="mt-4 rounded-3xl border border-zinc-800 bg-zinc-950 p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                <ResultBox label="現在" value={`${currentChampagne}本`} />
                <ResultBox label="達成率" value={`${champagneRate}%`} />
                <ResultBox
                  label="残り"
                  value={goal ? `${champagneRemaining}本` : "－"}
                />
              </div>
            </section>
            ...
            <section className="mt-4 rounded-3xl border border-zinc-800 bg-zinc-950 p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                <ResultBox label="現在" value={`${currentVisits}組`} />
                <ResultBox label="達成率" value={`${visitRate}%`} />
                <ResultBox
                  label="残り"
                  value={goal ? `${visitRemaining}組` : "－"}
                />
              </div>
            </section>
